                                                  ...
                                                  uint8_t nobordercases)
{
    ...
    if (nobordercases || (c+1 < width && r > 0)) {
        properties[index++] = top - ff_flif16_pixel_get(CTX_CAST(s), pixel, p, r-1, c+1);
    } else {
        properties[index++] = 0;
    }

    if (nobordercases || r > 1) {
        properties[index++] = ff_flif16_pixel_get(CTX_CAST(s), pixel, p, r-2, c) - top;
    } else {
        properties[index++] = 0;
    }

    if (nobordercases || c > 1) {
        properties[index++] = ff_flif16_pixel_get(CTX_CAST(s), pixel, p, r, c-2) - left;
    } else {
        properties[index++] = 0;
    }
 ...
 *    channel. This is used with the maniac integer reader.
...
static int flif16_write_frame(AVCodecContext *avctx, AVFrame *data)
{
    ...
    switch (avctx->pix_fmt) {
        ...
        default:
            av_log(avctx, AV_LOG_ERROR, "pixel format %d not supported\n", avctx->pix_fmt);
            return AVERROR_PATCHWELCOME;
    }
